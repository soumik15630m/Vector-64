...
namespace {
    ...
    bool check_against_rebuild(const Position& pos, int game, int ply) {
        ...
        if (rebuilt.material_wb() != pos.material_wb() || rebuilt.psqt_wb() != pos.psqt_wb()) {
        ...
        for (int s = 0; s < SQUARE_NB; ++s) {
            if (rebuilt.piece_on(static_cast<Square>(s)) != pos.piece_on(static_cast<Square>(s))) {
...
namespace {
    ...
    bool check_movegen_equivalence(Position& pos, std::mt19937_64& rng, int game, int ply) {
        MoveList full;
        ...
        for (int i = 0; i < captures.size(); ++i) {
            if (is_legal(nl, captures[i])) staged.push_back(captures[i]);
        }
        for (int i = 0; i < quiets.size(); ++i) {
            if (is_legal(nl, quiets[i])) staged.push_back(quiets[i]);
        }

        if (staged.size() != full.size()) {
            std::printf("FAIL game %d ply %d: staged %d vs full %d moves\n%s\n",
                        game, ply, staged.size(), full.size(), pos.toFEN().c_str());
            return false;
        }
        for (int i = 0; i < full.size(); ++i) {
            ...
            if (!is_pseudo_legal(pos, full[i])) {
                std::printf("FAIL game %d ply %d: is_pseudo_legal rejects legal move %d->%d\n%s\n",
                            game, ply, full[i].from_sq(), full[i].to_sq(), pos.toFEN().c_str());
                return false;
            }
        }
...
int main() {
    ...
    for (int game = 0; game < GAMES; ++game) {
        ...
        for (int ply = 0; ply < MAX_PLIES; ++ply) {
            ...
            const Move move = moves[static_cast<int>(rng() % static_cast<uint64_t>(moves.size()))];

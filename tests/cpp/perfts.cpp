...
uint64_t perft_multithreaded(Core::Position &pos, int depth) {
  ...
  for (int i = 0; i < moves.size(); ++i) {
    ...
    futures.push_back(
        std::async(std::launch::async, [cloned_pos, m, depth]() mutable {
          UndoInfo ui;
...
int run_epd_test_suite(const std::string &filepath, int max_depth_limit) {
  ...
  while (std::getline(file, line)) {
    ...
    for (int d = 1; d <= max_depth_limit; ++d) {
      ...
      if (idx != std::string::npos) {
        target_depth = d;
        expected_nodes =
            std::stoull(commands.substr(idx + search_str.length()));
      }
    }
    ...
    std::cout << "  Single  : " << nodes_st << " nodes | " << std::fixed
              << std::setprecision(3) << st_sec << "s | " << (nps_st / 1000000)
              << " MNPS\n";
    ...
    std::cout << "  Multi   : " << nodes_mt << " nodes | " << std::fixed
              << std::setprecision(3) << mt_sec << "s | " << (nps_mt / 1000000)
              << " MNPS";

    if (nodes_mt == expected_nodes && nodes_st == expected_nodes) {
      double speedup = st_sec / mt_sec;
      std::cout << "  [PASS] (Speedup: " << std::setprecision(2) << speedup
                << "x)\n";
      passed++;
    } else {
      std::cout << "  [FAIL]\n";
  ...
  std::cout << "SINGLE CORE : " << std::fixed << std::setprecision(3)
            << total_time_st << " seconds (" << (global_nps_st / 1000000)
            << " MNPS)\n";
  std::cout << "MULTI CORE  : " << std::fixed << std::setprecision(3)
            << total_time_mt << " seconds (" << (global_nps_mt / 1000000)
            << " MNPS)\n";
  std::cout << "SPEEDUP     : " << std::setprecision(2) << global_speedup
            << "x Faster\n";
  std::cout << "=========================================================\n";

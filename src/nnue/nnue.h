...
public:
  ...
  int evaluate_perspective(const Core::Position &pos,
...
private:
  ...
  void add_feature_vector(std::array<int16_t, HIDDEN_SIZE> &target,
                          uint32_t featureIndex) const;
  void add_feature_vector_scaled(std::array<float, HIDDEN_SIZE> &target,
                                 uint32_t featureIndex, float scale) const;
  void rebuild_accumulator(const Core::Position &pos,
                           Accumulator512 &accum) const;
  int infer_side_to_move(const Accumulator512 &accum, Core::Color stm) const;
  int infer_side_to_move_scaled(const Core::Position &pos,
                                Core::Color perspective) const;

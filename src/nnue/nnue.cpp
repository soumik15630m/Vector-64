...
constexpr std::streamoff align_to(std::streamoff offset,
                                  std::streamoff alignment) {
...
bool extract_json_float(const std::string &text, const std::string &key,
                        float &out) {
...
uint32_t halfkp_feature_index(Core::Square kingSquare,
                              Core::PieceType pieceType, Core::Color pieceColor,
...
bool Runtime::load_file(const std::string &path) {
  ...
  if (std::memcmp(header.magic, NETWORK_MAGIC, sizeof(header.magic)) != 0)
    return false;
  if (header.version != NETWORK_VERSION)
    return false;
  if (header.hiddenSize != HIDDEN_SIZE)
    return false;

  const size_t ftCount = static_cast<size_t>(HALF_KP_TOTAL_FEATURES) *
  ...
  const std::streamoff l2Bytes = static_cast<std::streamoff>(
      DENSE_L2_SIZE * DENSE_L1_SIZE * sizeof(int8_t));
  const std::streamoff outBytes =
      static_cast<std::streamoff>(DENSE_L2_SIZE * sizeof(int8_t));
  const std::streamoff biasBytes = static_cast<std::streamoff>(
      HIDDEN_SIZE * sizeof(int16_t) + DENSE_L1_SIZE * sizeof(int32_t) +
      DENSE_L2_SIZE * sizeof(int32_t) + sizeof(int32_t));
  ...
  const auto validate_zero_padding = [&](std::streamoff begin,
                                         std::streamoff end) -> bool {
    if (end < begin)
      return false;
    if (end == begin)
      return true;
    ...
    while (remaining > 0) {
      ...
      if (!read_exact(in, buffer.data(), chunk))
        return false;
  ...
  for (const SectionSpec &section : sections) {
    if (!validate_zero_padding(prevEnd, section.offset))
      return false;
  ...
  const auto seek_and_read = [&](const SectionSpec &section, void *dst,
                                 size_t bytes) -> bool {
    if (section.bytes != static_cast<std::streamoff>(bytes))
      return false;
    if ((section.offset % SECTION_ALIGNMENT) != 0)
      return false;

    in.seekg(section.offset, std::ios::beg);
    if (!in || in.tellg() != section.offset)
      return false;
    if (!read_exact(in, dst, bytes))
      return false;
    return in.tellg() == (section.offset + section.bytes);
  };

  if (!seek_and_read(sections[0], network.featureTransformWeights.data(),
                     network.featureTransformWeights.size() * sizeof(int16_t)))
    return false;

  if (!seek_and_read(sections[1], network.l1Weights.data(),
                     network.l1Weights.size() * sizeof(int8_t)))
    return false;
  if (!seek_and_read(sections[2], network.l2Weights.data(),
                     network.l2Weights.size() * sizeof(int8_t)))
    return false;
  if (!seek_and_read(sections[3], network.outWeights.data(),
                     network.outWeights.size() * sizeof(int8_t)))
    return false;

  in.seekg(sections[4].offset, std::ios::beg);
  if (!in || in.tellg() != sections[4].offset)
    return false;
  if (!read_exact(in, network.featureBias.data(),
                  network.featureBias.size() * sizeof(int16_t)))
    return false;
  if (!read_exact(in, network.l1Bias.data(),
                  network.l1Bias.size() * sizeof(int32_t)))
    return false;
  if (!read_exact(in, network.l2Bias.data(),
                  network.l2Bias.size() * sizeof(int32_t)))
    return false;
  if (!read_exact(in, &network.outBias, sizeof(network.outBias)))
    return false;
  if (in.tellg() != (sections[4].offset + sections[4].bytes))
    return false;
...
void Runtime::add_feature_vector(std::array<int16_t, HIDDEN_SIZE> &target,
                                 uint32_t featureIndex) const {
  if (featureIndex >= HALF_KP_TOTAL_FEATURES)
    return;
  const size_t base =
      static_cast<size_t>(featureIndex) * static_cast<size_t>(HIDDEN_SIZE);
  ...
  for (int i = 0; i < HIDDEN_SIZE; i += 16) {
    ...
    __m256i v_tgt =
        _mm256_load_si256(reinterpret_cast<const __m256i *>(tgt + i));
    // adds_epi16 performs saturated addition, which is standard for NNUE
    _mm256_store_si256(reinterpret_cast<__m256i *>(tgt + i),
                       _mm256_adds_epi16(v_tgt, v_src));
  }
#else
...
void Runtime::rebuild_accumulator(const Core::Position &pos,
                                  Accumulator512 &accum) const {
  accum.clear();
  if (!loaded_)
    return;
  ...
  for (int c = Core::WHITE; c <= Core::BLACK; ++c) {
    ...
    for (int pt = Core::PAWN; pt <= Core::QUEEN; ++pt) {
      ...
      while (bb) {
        ...
        const uint32_t whiteIdx =
            halfkp_feature_index(whiteKingSq, static_cast<Core::PieceType>(pt),
                                 color, sq, Core::WHITE);
        const uint32_t blackIdx =
            halfkp_feature_index(blackKingSq, static_cast<Core::PieceType>(pt),
                                 color, sq, Core::BLACK);
...
void Runtime::add_feature_vector_scaled(std::array<float, HIDDEN_SIZE> &target,
...
int32_t dot_i8(const int8_t *RESTRICT act, const int8_t *RESTRICT w, int n) {
  ...
  sum128 =
      _mm_add_epi32(sum128, _mm_shuffle_epi32(sum128, _MM_SHUFFLE(2, 3, 0, 1)));
  sum128 =
      _mm_add_epi32(sum128, _mm_shuffle_epi32(sum128, _MM_SHUFFLE(1, 0, 3, 2)));
  int32_t sum = _mm_cvtsi128_si32(sum128);
  for (; i < n; ++i)
    sum += static_cast<int32_t>(w[i]) * static_cast<int32_t>(act[i]);
  return sum;
#else
  int32_t sum = 0;
  for (int i = 0; i < n; ++i)
...
void transform_x0(const int16_t *RESTRICT us, const int16_t *RESTRICT them,
                  int8_t *RESTRICT x0, int n) {
#if defined(__AVX2__)
...
int Runtime::infer_side_to_move(const Accumulator512 &accum,
                                Core::Color stm) const {
  ...
  for (int out = 0; out < DENSE_L1_SIZE; ++out) {
    const size_t row =
        static_cast<size_t>(out) * static_cast<size_t>(HIDDEN_SIZE);
    int32_t sum =
        network_.l1Bias[static_cast<size_t>(out)] +
        dot_i8(x0.data(), network_.l1Weights.data() + row, HIDDEN_SIZE);
    x1[static_cast<size_t>(out)] =
        relu_i8(clamp_sym_i8(sum / DENSE_TO_DENSE_SCALE));
  }

  for (int out = 0; out < DENSE_L2_SIZE; ++out) {
    const size_t row =
        static_cast<size_t>(out) * static_cast<size_t>(DENSE_L1_SIZE);
    int32_t sum =
        network_.l2Bias[static_cast<size_t>(out)] +
        dot_i8(x1.data(), network_.l2Weights.data() + row, DENSE_L1_SIZE);
    x2[static_cast<size_t>(out)] =
        relu_i8(clamp_sym_i8(sum / DENSE_TO_DENSE_SCALE));
  }
...
bool Runtime::self_test_quantized_inference() {
  ...
  auto infer_scalar = [](const Network &net, const Accumulator512 &accum,
                         Core::Color stm) -> int {
    const auto &us = stm == Core::WHITE ? accum.white : accum.black;
    const auto &them = stm == Core::WHITE ? accum.black : accum.white;
    ...
    for (int o = 0; o < DENSE_L2_SIZE; ++o) {
      ...
      for (int i = 0; i < DENSE_L1_SIZE; ++i)
        sum +=
            static_cast<int32_t>(net.l2Weights[o * DENSE_L1_SIZE + i]) * x1[i];
  ...
  auto rndW = [&](int lo, int hi) {
    return static_cast<int>(
        lo + static_cast<int>(next() % static_cast<uint64_t>(hi - lo + 1)));
  };

  Runtime rt;
  rt.network_.featureTransformWeights.clear();
  for (int trial = 0; trial < 64; ++trial) {
    for (auto &w : rt.network_.l1Weights)
      w = static_cast<int8_t>(rndW(-127, 127));
    for (auto &w : rt.network_.l2Weights)
      w = static_cast<int8_t>(rndW(-127, 127));
    for (auto &w : rt.network_.outWeights)
      w = static_cast<int8_t>(rndW(-127, 127));
    for (auto &b : rt.network_.l1Bias)
      b = rndW(-4000, 4000);
    for (auto &b : rt.network_.l2Bias)
      b = rndW(-4000, 4000);
    ...
    for (Core::Color stm : {Core::WHITE, Core::BLACK}) {
      ...
      if (a != b) {
        std::fprintf(stderr,
                     "[nnue self-test] trial %d stm %d: simd %d scalar %d\n",
...
int Runtime::infer_side_to_move_scaled(const Core::Position &pos,
                                       Core::Color perspective) const {
  const Core::Bitboard whiteKing = pos.pieces(Core::KING, Core::WHITE);
  const Core::Bitboard blackKing = pos.pieces(Core::KING, Core::BLACK);
  if (whiteKing == 0 || blackKing == 0)
    return 0;
  ...
  for (int i = 0; i < HIDDEN_SIZE; ++i) {
    const float bias =
        static_cast<float>(network_.featureBias[static_cast<size_t>(i)]) *
        scales_.featureTransformBias;
  ...
  for (int c = Core::WHITE; c <= Core::BLACK; ++c) {
    ...
    for (int pt = Core::PAWN; pt <= Core::QUEEN; ++pt) {
      ...
      while (bb) {
        ...
        add_feature_vector_scaled(whiteAcc, whiteIdx,
                                  scales_.featureTransformWeight);
        add_feature_vector_scaled(blackAcc, blackIdx,
                                  scales_.featureTransformWeight);
      }
    }
  }
  ...
  for (int i = 0; i < HIDDEN_SIZE; ++i) {
    const float centered =
        (us[static_cast<size_t>(i)] - them[static_cast<size_t>(i)]) /
        static_cast<float>(FEATURE_TO_DENSE_SCALE);
    x0[static_cast<size_t>(i)] = std::max(0.0f, centered);
  }

  for (int out = 0; out < DENSE_L1_SIZE; ++out) {
    float sum = static_cast<float>(network_.l1Bias[static_cast<size_t>(out)]) *
                scales_.l1Bias;
    const size_t row =
        static_cast<size_t>(out) * static_cast<size_t>(HIDDEN_SIZE);
    for (int i = 0; i < HIDDEN_SIZE; ++i) {
      sum += (static_cast<float>(
                  network_.l1Weights[row + static_cast<size_t>(i)]) *
              scales_.l1Weight) *
             x0[static_cast<size_t>(i)];
    }
    x1[static_cast<size_t>(out)] =
        std::max(0.0f, sum / static_cast<float>(DENSE_TO_DENSE_SCALE));
  }

  for (int out = 0; out < DENSE_L2_SIZE; ++out) {
    float sum = static_cast<float>(network_.l2Bias[static_cast<size_t>(out)]) *
                scales_.l2Bias;
    const size_t row =
        static_cast<size_t>(out) * static_cast<size_t>(DENSE_L1_SIZE);
    for (int i = 0; i < DENSE_L1_SIZE; ++i) {
      sum += (static_cast<float>(
                  network_.l2Weights[row + static_cast<size_t>(i)]) *
              scales_.l2Weight) *
  ...
  for (int i = 0; i < DENSE_L2_SIZE; ++i) {
    out += (static_cast<float>(network_.outWeights[static_cast<size_t>(i)]) *
            scales_.outWeight) *
...
bool Runtime::try_load_sidecar_scales(const std::string &path) {
  ...
  candidates.emplace_back(std::filesystem::path(path).filename().string() +
                          ".meta.json");
  ...
  for (const auto &candidate : candidates) {
    ...
    QuantScales parsed{};
    if (!extract_json_float(text, "feature_transform_weight",
                            parsed.featureTransformWeight))
      continue;
    if (!extract_json_float(text, "feature_transform_bias",
                            parsed.featureTransformBias))
      continue;
    if (!extract_json_float(text, "dense1_weight", parsed.l1Weight))
      continue;
    if (!extract_json_float(text, "dense1_bias", parsed.l1Bias))
      continue;
    if (!extract_json_float(text, "dense2_weight", parsed.l2Weight))
      continue;
    if (!extract_json_float(text, "dense2_bias", parsed.l2Bias))
      continue;
    if (!extract_json_float(text, "output_weight", parsed.outWeight))
      continue;
    if (!extract_json_float(text, "output_bias", parsed.outBias))
      continue;

    if (parsed.featureTransformWeight <= 0.0f ||
        parsed.featureTransformBias <= 0.0f)
      continue;
    if (parsed.l1Weight <= 0.0f || parsed.l1Bias <= 0.0f)
      continue;
    if (parsed.l2Weight <= 0.0f || parsed.l2Bias <= 0.0f)
      continue;
    if (parsed.outWeight <= 0.0f || parsed.outBias <= 0.0f)
      continue;
...
int Runtime::material_proxy_stm(const Core::Position &pos) const {
  static constexpr int VALUES[Core::PIECE_TYPE_NB] = {0,   100, 320, 330,
                                                      500, 900, 0};
  ...
  for (int pt = Core::PAWN; pt <= Core::KING; ++pt) {
    ...
    white += Core::popcount(
                 pos.pieces(static_cast<Core::PieceType>(pt), Core::WHITE)) *
             value;
    black += Core::popcount(
                 pos.pieces(static_cast<Core::PieceType>(pt), Core::BLACK)) *
             value;
  }
...
int Runtime::evaluate_perspective(const Core::Position &pos,
                                  Core::Color perspective) const {
  if (!loaded_)
    return material_proxy_stm(pos);

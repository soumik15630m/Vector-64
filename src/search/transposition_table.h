...
public:
  explicit TranspositionTable(size_t mb = 8) { resize_mb(mb); }

  ~TranspositionTable() { free_buckets(); }
  ...
  void store(uint64_t key, int score, Core::Move move, int depth, TTBound bound,
             int ply, int eval) {
    ...
    if (!sameKey || bound == BOUND_EXACT || depth + 4 >= e.depth) {
      ...
      e.eval =
          static_cast<int16_t>(std::clamp(eval, TT_EVAL_NONE, INT16_MAX + 0));
      e.move16 = move.raw();
